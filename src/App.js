...
export default class App extends Component {
  ...
  save = () => {
    const result = Array.from(document.querySelector("form").elements).filter(function(item) {
  ...
  render() {
    const {menuItems, selectedMenuItem, configHasErrors, configErrorMessage} = this.state;
    ...
    return (<Grid style={{
        ...
        width: "height: 90vh",
        overflow: "none"
      }} middle="true" aligned="true" center="true">
      {/* <DevTools /> */}
      ...
      <Grid.Row columns="equal" stretched="stretched" centered="centered" padded="true" style={{
          backgroundColor: background
        }}>
        <Grid.Column>
          <Segment basic="basic">
            <LeftNav selectedMenuItem={selectedMenuItem} menuItems={menuItems} onMenuSelect={this.onMenuSelect}/>
          </Segment>
        </Grid.Column>

        {/* Main Content */}
        <Grid.Column>
          <MainContent options={options} menuItems={menuItems} selectedMenuItem={selectedMenuItem}/>
        </Grid.Column>
        <Grid.Column>
          <Segment basic="basic">
            <ImportExport/>
...
const ImportExport = () => {
  return (<div>
    <Button primary="primary" fluid="fluid" onClick={this.save}>
      Import
    </Button>

    <Divider/>
    <Button secondary="secondary" fluid="fluid" onClick={this.save}>
      Export
    </Button>
  </div>);
};
    ...
    <Menu>
      ...
      <Menu.Menu position="right" padded="true">
        <Menu.Item name="wiki" onClick={this.handleItemClick}>
          <a href="https://github.com/mehtadone/PTFeeder/wiki" target="_blank" rel="noopener noreferrer">
            Wiki
          </a>
        </Menu.Item>

        <Menu.Item name="videos" onClick={this.handleItemClick}>
          <a href="https://github.com/mehtadone/PTFeeder/wiki/Videos" target="_blank" rel="noopener noreferrer">
            Videos
          </a>
        </Menu.Item>

        <Menu.Item name="support" onClick={this.handleItemClick}>
          <a href="https://github.com/mehtadone/PTFeeder/issues" target="_blank" rel="noopener noreferrer">
            Support
          </a>
        </Menu.Item>
        <Menu.Item>
...
const LeftNav = ({selectedMenuItem, menuItems, onMenuSelect}) => {
  return (<Grid.Column width={4} align="center">
    <Sticky>
      <Menu vertical="vertical" style={{
          fontFamily: "Poppins",
          fontSize: "16px",
          boxShadow: "none",
          border: "none",
        ...
        {
          menuItems && menuItems.map(title => (<Menu.Item name={title} key={title} active={selectedMenuItem === title} onClick={() => onMenuSelect(title)} style={{
              color: textColour
            }}>
            {title}
          </Menu.Item>))
